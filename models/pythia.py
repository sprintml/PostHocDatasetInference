...
class PythiaClassification:
    def __init__(self, model_name='EleutherAI/pythia-410m', 
        ...
        # Handle padding token for Pythia models
    ...
    def initialize_model(self):
        ...
        if self.from_pretrained:
            ...
            base_model_config = AutoConfig.from_pretrained(self.model_name, num_labels=self.num_labels)
            base_model = AutoModelForSequenceClassification.from_pretrained(self.model_name, config=base_model_config)
            
            # For Pythia, we can either use a similar approach as with GPT2 to extract specific layers,
            # or simply fine-tune the entire pre-trained model
            self.model = base_model
        else:
            # Define a custom small Pythia-like config for training from scratch
            # Note: Pythia configurations differ from GPT2, adjust accordingly
            model_config = AutoConfig.from_pretrained(self.model_name)
            model_config.num_labels = self.num_labels
            
            # For a smaller custom model, you can modify the configuration
            if "pythia" in self.model_name.lower():
                # You can create a smaller version of the model by reducing layers
                model_config.num_hidden_layers = 2  # Reduce number of layers
        ...
        if self.use_lora:
            ...
            lora_config = LoraConfig(
                r=4,  # LoRA rank
                lora_alpha=4,
                # For Pythia, target modules might be different than GPT2
                # You may need to inspect the model to identify the right layers
                # target_modules=["query_key_value", "classifier"],
                lora_dropout=0.1,
                bias="none",
            )
