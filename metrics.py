...
def raw_probs_batch(model, tokenizer, example_list):
    '''
    This function takes a list of strings and returns the loss values for each token in the string
    input:
        model: the language model
...
def raw_probs(model, tokenizer, example_list, batch_size = 32):
    '''
    This function takes a list of strings and returns the loss values for each token in the string
    input:
        model: the language model
...
def ppl_ratio(loss_list, reference_list):
    ...
    for (entry, entry_ref) in zip(loss_list, reference_list):
        ...
        if type(entry_ref) == list:
            ...
            mean_ref = entry_ref
        # calculate the ratio
        eps = 1e-10 # avoid zero devision
        ratio = (mean_model+eps)/(mean_ref+eps)
        ratios.append(ratio)
...
reference_model_registry = {
    ...
    # "llama3-8b_ckpt": "meta-llama/Meta-Llama-3-8B",
...
def aggregate_metrics(model, tokenizer, dataset, dataset_col, metric_list, args, batch_size = 32, prefix=False, ref_ckpt=None, data_dir=None):
    '''
    ...
    input:
        ...
        dataset_col: which column of the dataset is the text domain 
        metric_list: a list of metrics to calculate

    output:
        metrics: a dictionary of metrics
    '''
    # extract texts from dataset and get losses
    example_list = dataset[dataset_col]
    if dataset_col == 'paraphrase':
        example_list = [paraphrases[0] for paraphrases in example_list]
    if prefix:
        prefix_list = dataset['prefix']
        example_list = [prefix+example for prefix, example in zip(prefix_list, example_list)]
    print(f'example_list: {len(example_list)}')
    ...
    if "reference_model" in metric_list:
        ...
        for model_name in reference_model_registry:
            ...
            if "_ckpt" in model_name:
                local_ckpt = ref_ckpt.replace("model_name", hf_path.split("/")[-1]) # in the path, "model_name" is provided as a placeholder
            else:
                local_ckpt = None

            model, tokenizer = prepare_model(hf_path, cache_dir=args.cache_dir, local_ckpt=local_ckpt, data_dir=data_dir)
            
            reference_list = raw_values(model, tokenizer, example_list, batch_size = 32)
            print(len(reference_list))
            metrics[f"ref_ppl_{model_name}"] = perplexity(reference_list)
            metrics[f"ref_ppl_ratio_{model_name}"] = ppl_ratio(loss_list, reference_list)
            print(f'ref_ppl_ratio_{model_name}: {len(metrics[f"ref_ppl_ratio_{model_name}"])}')
